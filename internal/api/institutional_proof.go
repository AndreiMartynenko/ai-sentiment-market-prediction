...
type institutionalProofRequest struct {
	Signal any `json:"signal" binding:"required"`
}

func PublishInstitutionalSignalProof(mlServiceURL string) gin.HandlerFunc {
	mlBase := strings.TrimRight(mlServiceURL, "/")
	client := &http.Client{Timeout: 25 * time.Second}

	return func(c *gin.Context) {
		var reqBody institutionalProofRequest
		...
		mlReq, err := http.NewRequest(http.MethodPost, mlBase+"/signal/institutional/proof", bytes.NewReader(b))
		if err != nil {
